...
def hack_ciphertext_JS(ciphertext):
    ...
    while key_stability != 1000:
        ...
        print(key_tmp)
        print(temp_total_distance, total_distance)
        print(key_stability)
...
p_400 = "Утром город просыпается медленно, но уверенно. Первые лучи солнца пробиваются сквозь облака, освещая пустые улицы. Ветер тихо шепчет среди деревьев, а прохожие спешат по своим делам, не замечая красоты вокруг. Где-то вдали слышится шум трамвая, а запах свежего хлеба манит из открытых дверей пекарен. Это начало нового дня, полного ожиданий и надежд."
d = decrypt_ciphertext(p_400, key)
